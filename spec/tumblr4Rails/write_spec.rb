...
describe Tumblr4Rails::Write do
  ...
  def mock_options_hash
    mock_hash = mock("Options")
    mock_hash.stub!(:email).and_return("test@test.ca")
    mock_hash.stub!(:password).and_return("xxxx")
    mock_hash
  end
  ...
  before(:each) do
    ...
    @gateway.stub!(:post).and_return(@resp)
  ...
  describe "create audio post" do
    ...
    it "should receive args with the proper contents" do
      self.should_receive(:create_post).with(
        hash_including(:type => :audio, :source => "http://www.google.ca/something.mp3")
      ).and_return(@resp)
      self.create_audio_post("http://www.google.ca/something.mp3")
    end
    
    it "should receive args with additional arguments included" do
      self.should_receive(:create_post).with(
        hash_including(:generator => "Test", :private => "0", :caption => "A Song")
      ).and_return(@resp)
      self.create_audio_post("http://www.google.ca/something.mp3", nil,
        "A Song", {:generator => "Test", :private => "0"})
    end
    
    it "should raise an exception if neither source nor data are set" do
      lambda {
        self.self.create_audio_post
      }.should raise_error
    end
    
    it "should raise an exception if both source and data are set" do
      lambda {
        self.self.create_audio_post("http://www.google.ca/song.mp3", "dfdfsdfdf")
      }.should raise_error
    end
  ...
  describe "create video post" do
    ...
    it "should receive args with the proper contents" do
      self.should_receive(:create_post).with(
        hash_including(:type => :video, :embed => "http://www.google.ca/something.mpeg")
      ).and_return(@resp)
      self.create_video_post("http://www.google.ca/something.mpeg")
    end
    
    it "should receive args with additional arguments included" do
      self.should_receive(:create_post).with(
        hash_including(:generator => "Test", :private => "0", :caption => "A Video", :title => "A Video")
      ).and_return(@resp)
      self.create_video_post("http://www.google.ca/something.mpg", nil,
        "A Video", "A Video", {:generator => "Test", :private => "0"})
    end
    
    it "should raise an exception if neither embed nor data are set" do
      lambda {
        self.self.create_video_post
      }.should raise_error
    end
    
    it "should raise an exception if both embed and data are set" do
      lambda {
        self.self.create_video_post("http://www.google.ca/song.mpg", "dfdfsdfdf")
      }.should raise_error
    end
  ...
  describe "create_post" do
    ...
    it "should post to the API using the post method of the gateway object" do
      @gateway.should_receive(:post).with(Tumblr4Rails.configuration.write_url,
        hash_including(create_options)).and_return(create_mock_http_response)
      do_create_post(create_options)
    end
