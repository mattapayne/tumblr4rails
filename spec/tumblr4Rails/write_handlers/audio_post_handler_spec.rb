...
describe Tumblr4Rails::WriteOptions::AudioPostHandler do
  
  def handler(options)
    Tumblr4Rails::WriteOptions::AudioPostHandler.new(options)
  end
  
  def create_options(other={})
    {:type => :audio, :data => Tumblr4Rails::Upload.new("text.xml", "ffddf"), 
      :caption => "test", :email => "test@test.ca", :password => "dfsdfd",
  ...
  it "should remove any entries with nil values" do
    options = create_options(:caption => nil)
    h = handler(options)
    result = h.process!
  ...
  it "should raise an exception if there is no filename" do
    options = create_options(:data => Tumblr4Rails::Upload.new("", "dfdsfd"))
    lambda {handler(options).process!}.should raise_error
  end
  
  it "should raise an exception if the data param is not present" do
    options = create_options_except(:data)
    lambda {handler(options).process!}.should raise_error
  end
  ...
  it "should raise an exception if the data param is nil" do
    options = create_options(:data => nil)
    lambda {handler(options).process!}.should raise_error
  end
